...
import { isArray, isObject, isNaN, isFunction } from "lodash-es";
...
const createSpecificShallowEqual = <TProps>(
   ...
   */
  return (
    props: SpecificShallowEqualType<TProps>,
    nextProps: SpecificShallowEqualType<TProps>
  ) => {
    return keysToTest.every(keyToTest => {
      ...
      if (Iterable.isIterable(currentVal) || Iterable.isIterable(nextVal)) {
        ...
        nextVal !== undefined &&
        nextVal !== null &&
        isValidElement(nextVal)
      ) {
        const currentString = ReactDOMServer.renderToStaticMarkup(
          currentVal as ReactElement<any>
        );
        ...
        if (isFunction(nextVal)) {
          ...
          (isArray(nextVal) || isObject(nextVal) || isNaN(nextVal)) &&
          !(nextVal instanceof File)
        ) {
          throw new Error(
            `Specific shallow equal does not support plain old JS objects, Arrays and NaN: prop ${keyToTest} is a ${typeof nextVal}`
          );
        }
        return currentVal === nextVal;
      }
